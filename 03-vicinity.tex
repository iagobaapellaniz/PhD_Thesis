...
Prime examples of such states are GHZ states \citep{Greenberger1990}, which have already been realized experimentally many times \citep{Bowmeester1999, Pan2000, Zhao2003, Lu2007, Gao2010, Sackett2000, Monz2011, Leibfried2012}.
Hence, these states have attracted the interest of the community.

One of the figures of merit of this such unpolarized, but still useful states is the so-called unpolarized Dicke state \citep{}, which consists of, on its $z$-axis  representation, an equal number of qubits pointing up and pointing down while the whole state is at the symmetric subspace.
...
Those properties attracted our attention to those states since the quantum Fisher information, used to characterize the metrological usefulness, is the convex-roof of $4\varian{J_{\bs{n}}}$, where $\bs{n}$ is the unitary vector pointing in the direction of the magnetic field.
...
\subsection{Unpolarized states for magnetometry}

Having unpolarized states for magnetometry has been shown useful in Section~\ref{}.
...
Say that in this case the initial state is an unpolarized Dicke state of aligned with the $x$-axis.
\begin{figure}[htp]
  \centering
  \includegraphics[scale=.65]{img/plots/VD_evolution_of_dicke.pdf}
  \caption[Sequence of Dicke state evolution]{Sequence of the evolution of an unpolarized Dicke state of 16 qubits for $\Theta=\{i\pi/6\}_{i=0}^4$. Bloch spheres representing the Husimi quasi-probabilistic distribution of the state, and below PDF of the $J_x$ positive-operator valued measure (POVM) for each step of the sequence}
