...
It turns out that the unpolarized states are the most adequate states to bite the Heisenberg limit, as it was shown in the Section~\ref{sec:bg-quantum-metrology}.
Hence, these states have attracted the interest of the community.

One of the figures of merit of this such unpolarized, but still useful, states is the so-called unpolarized Dicke state \cite{Dicke1954}, which consists of, in its $z$-axis  representation, an equal number of qubits pointing up and pointing down while the whole state is in the symmetric subspace.
...
For this case, a criterion to detect the metrological usefulness of some of these states has been derived for symmetric systems \cite{Zhang2014}.
...
Based on Eq.~\eqref{eq:bg-entanglement-depth-for-qfi}, we can find entanglement depth of the systems as follows.
...
\ee
which coincides exactly with the inverse quantum Fisher information for such state Eq.~\eqref{eq:vd-qfi-for-pure-dicke} \cite{Luecke2011}.
...
In reference \cite{Luecke2014}, it is produced in the laboratory a state with the proper characteristics of an unpolarized Dicke state, small variance in one direction, say $x$, and a very large variance in the perpendicular directions to the $x$-axis.
...
It is worth to study the case appearing in Ref.~\cite{Krischek2011} and apply our methods such that we extract conclusions about the metrological usefulness of the state.
...
\begin{figure}[htp]
  ...
  (solid) Precision bound based in the Eq.~\eqref{eq:vd-precision-bound-for-second-moments}.
  One can see that decreasing further the uncertainty in $\expect{J_x^2}$ can improve the bound significantly.
  (white-point) Point for the experimental data with the corresponding errors.
  Even if the point is slightly below the 4-particle entanglement level, we can say that with only two second moments, we characterize the state in such a way that it does not disagree excessively with the original prediction when fourth moments were also included and 4-particle entanglement was witnessed for the system.}
