...
\ee
where $k$ are elements of the set of all possible unique permutations of $N$ elements of 2 kinds, $\sigma_\text{s}$, see Appendix~\ref{app:angular-subspaces} for more information about the notation used in Eq.~\eqref{eq:vd-unpolarised-dicke}.
Note that in Eq.~\eqref{eq:vd-unpolarised-dicke} we omit the subscript giving the number of $\ket{1}$'s which is the notation we will follow in this chapter.
...
One of the characteristics of state Eq.~\eqref{eq:vd-unpolarised-dicke} is that it is an eigenstate of the collective operator $J_l$ with corresponding eigenvalue equal to zero.
