...
Finally, it is important to note that if the operators $W_k$ are permutationally invariant and the eigenstate with the maximal eigenvalue in Eq.~\eqref{eq:lt-legendre-for-qfi-simplified} is non-degenerate, then, we can do the computations on the symmetric subspace only.
...
Note that estimating $\qfif{\rho, J_z}$ based on $F_{\text{Dicke}}$ was possible for 40 qubits [TD: Ask geza for the data points for 40 particles] for Figure~\ref{fig:lt-plots-for-fidelities}-(b), since we carried out the calculations for the symmetric subspace.
...
\be
  ...
  \begin{aligned}
    &\frac{\binom{N/2}{m/2}^2\binom{N}{N/2}}{2^N\binom{N}{m}} && \text{for even }m,\\
    &0 && \text{for odd }m,
  \end{aligned}
  \right.
...
\be
  ...
  \bound{N}\approx \frac{1}{\gamma}\frac{\mathcal{J}_{N}}{\mathcal{J}_{N'}} \bound{\text{sym},N'} =\frac{\expect{J_x^2}_N+\expect{J_y^2}_N+\expect{J_z^2}_N}{\mathcal{J}_{N'}}\bound{\text{sym},N'}.
\ee
