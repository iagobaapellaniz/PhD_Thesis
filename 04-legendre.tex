...
\begin{figure}
  ...
  \caption{(a) Analytical solution of the bound $\bound{\mathcal{F}}$ for different values of the fidelity with respect to the GHZ state.
...
\begin{table}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Fidelity values and the corresponding bound for the QFI for several experiments with Dicke states and GHZ states.
...
\begin{figure}
  ...
  \caption{[TD: Change vertical label to $\bound{\textnormal{sym}}$] (Color line) Lower bound on the QFI based on $\expect{J_y}$ and $\varian{J_x}$ obtainde for the symmetric subspace for different particle numbers $N'$.}
...
We now give more details of the calculation. We were able to carry out the optimizations up to $N'=2300$ with a usual laptop computer using MATLAB programing language\footnote{For MATLAB R2015a, see \url{http://www.mathworks.com}.}.
...
Then, to obtain the results for the original non-symmetric case, notice that the convex nature of the $\bound{N}$, which is the bound to be computed for the general case, implies
\be
...
Notice that $\bs{J}$ is a shorthand of $J_x^2+J_y^2+J_z^2$.
