...
The problem of getting a lower bound for a convex function on the states having already some expectation values was studied by O. G\"uhne {\it et al.} and J. Eisert {\it et al.} in Ref. \citep{Guehne2007, Eisert2007}, mainly for entanglement measures. We first review the state-of-the-art solution for this problem.
...
\be
  \label{eq:lt-for-convex-function}
  \hat{g}(rW)=\sup_{\rho}\{\expect{rW}_\rho - g(\rho)\}.
\ee
This method has been used to compute entanglement measures, as we mentioned before.

Following the theory one can find that in the case on which the convex function $g(\rho)$ is defined as convex roof over all possible decompositions of the state, the optimisation of Eq.~\ref{eq:lt-for-convex-function} can be reduce to an optimisation over pure states, thus simplifying the task.
However, an optimization over pure states is only feasible for small systems when the problem must be solved numerically.
Many entanglement measures extended from been defined for pure states to mixed state using this process \citep{}.
