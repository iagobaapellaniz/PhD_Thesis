...
In the following we will omit the second subscript and we use the notation $\ket{D_N}_{x}\equiv \ket{D_{N,N/2}}_{x}$, where we even may skip the subscript $x$ since this Dicke state will be always in the center of our attention, the unpolarized Dicke state perpendicular to the magnetic field in this case along the $z$-direction.
...
In order to maximize the overlap with the Dicke state $\ket{D_N}_{x}$, we have to look for symmetric eigenstates of $J_z$.
These are the symmetric Dicke states in the $z$-basis $\ket{D_{N,m}}_z$.
Then using the formula
\be
  \sum_k \binom{N}{k}\binom{N}{q-k}(-1)^k = \lcor
  \begin{aligned}
    &\binom{N}{q/2}(-1)^{q/2},&& \text{for even }N \text{ and }q,\\
    &0, && \text{for odd }N.
  \end{aligned}
  \right.
\ee
[TD: check the above equation] one finds that the squared overlap is given by
\be
  |\braopket{D_{N,m}}{_z}{D_N}_x|^2 = \lcor
  \begin{aligned}
    &\frac{\binom{N/2}{m/2}^2\binom{N}{N/2}}{2^N\binom{N}{m}} ,&& \text{for even }N \text{ and }q,\\
    &0, && \text{for odd }N,
  \end{aligned}
  \right.
\ee
which maximal is for even $N$ and $m$ when $m=N$ or $m=0$, thus the totally or anti-totally polarized states respectively.
