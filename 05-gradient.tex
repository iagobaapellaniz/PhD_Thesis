...
The achievable precision in terms of the number of particles scales as $\varinv{\theta}\sim N$ and $\varinv{\theta}\sim N^2$ for SNS and HS respectively.
...
Note also that even if the first term cannot overcome the SNS, in the second term the covariance is multiplied by QFI for estimating the homogeneous field and therefore this concrete term can make the bound, for extremely correlated particle positions, to scale as HS.
...
\be
  \varinv{b_1}_{\text{SNL}} \leqslant  \sigma^2 4N j^2,
...
Since both states are unpolarized and PI, we have that $\expect{J_z}=0$ and $\expect{j_z^{(n)}}=0$ for both cases
