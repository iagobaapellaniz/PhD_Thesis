...
\be
  \qfif{\ket{\psi}^{(\text{L})}\otimes\ket{\psi}^{(\text{R})},a( \mtxid^{(\text{L})}\otimes J_{z}^{(\text{R})}-J_{z}^{(\text{R})}\otimes \mtxid^{(\text{L})})}=2a^2\qfif{\ket{\Psi},J_z},
\ee
where we used the general rule Eq.~\eqref{eq:bg-qfi-additive-for-tensor-product} and that any scalar multiplying the second argument of the QFI can be extracted outside the function squared were used.
Note that
\begin{multline}
\label{eq:separability-on-two-ensembles}
F[\varrho_1^{(L)} \varrho_2^{(R)}, A^{(L)} \mtxid^{(R)}+\mtxid^{(L)} A^{(R)},  B^{(L)} \mtxid^{(R)}+\mtxid^{(L)} B^{(R)}]\\ = F[\varrho_{\rm L}, A,B] + F[\varrho_{\rm R}, A, B].
\end{multline}

We analyze first how the bound would look like for states sensitive to the homogeneous field, and then for states insensitive to it.
Note that the single point correlation function for "L" and "R" particles is $a$ and $-a$ respectively and the two point correlation function is $a^2$ for both. Thus, in the case of computing the bound for the states sensitive to the homogeneous fields, we have that $F_{01}^{(\text{L})} = -F_{01}^{(\text{R})}$ whereas the other two matrix elements we have to compute are equal for both subspaces "L" and "R".
...
(\Delta b_1)^{-2}\leq&\, F_{11}+\frac{(F_{01})^2}{F_{00}}\\
 =&\,F_{11}^{(\text{L})}+F_{11}^{(\text{R})}+\frac{(F_{01}^{(\text{L})}+F_{01}^{(\text{R})})^2}{F_{00}^{(\text{L})}+F_{00}^{(\text{R})}}\\
 =&\,2F_{00}^{(\text{L})} +\frac{(F_{01}^{(\text{L})}-F_{01}^{(\text{R})})^2}{2F_{00}^{(\text{L})}}\\
 =&\,2F_{11}^{(\text{L})}\\
 =&\, 2 a^2 F[\varrho^{(L)}, J_z^{(L)}],
\end{split}
\ee
where we use in the first line the identity~(\ref{eq:separability-on-two-ensembles}) and in the last line we extract the common factor $a^2$ and we use the linearity on the arguments the QFI.
% WHAT IS THIS?: has to obtain $F[\varrho^{(L)}, J_z^{(L)}]$.
Note that this is the same precision bound we will obtain for states insensitive to the homogeneous fields.
The reader may notice that this bound relates how good the state on the "L" or "R" subsystem is in sensing the homogeneous field with the precision achievable for the gradient parameter.
This is reasonable because the state in "L" is not interacting neither correlated with "L".
...
\be F[\ket{\Psi_{\rm sep}^{(\rm L)}}\otimes\ket{\Psi_{\rm sep}^{(\rm R)}},J_z^{(L)} \mtxid^{(R)}+\mtxid^{(L)} J_z^{(R)}]\le 4Nj^2,\ee
