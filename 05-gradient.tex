...
We can also connect our results to entanglement theory \citep{Werner1989,Horodecki2009,Guehne2009}.
...
\begin{split}
  (\Delta b_1)^{-2}\leq& \sum_{n,m}^N \int d^N\bs{x}P_n(\bs{x})x_nx_m \qfif{\rho_{\rm s}, j_z^{(n)}, j_z^{(m)}}\\
...
\begin{split}
  (\Delta b_1)^{-2}\leq \,&\sum_{n,m}^Nnma^2\qfif{\ket{j}_y^{\otimes N}, j_z^{(n)},j_z^{(m)}}\\
