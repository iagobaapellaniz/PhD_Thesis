...
This approach was not taken in Ref.~\cite{}, for instance, where with a separable state $N^2$ could be achieved or where with the optimal state a $N^4$ scaling could be achieved.
In that case this was due to fact that adding more particles to the system the effective length of it increases, and it becomes more sensitive to the gradient field merely because the system reaches further away as it increases in size.
...
Note that for the simplification of the QFI, we use the Dirac delta appearing in Eq.~\eqref{eq:gm-simplify-h1-matrix-els}, and the state defined by Eq.~\eqref{eq:gm-eigendecomposition-of-state}.
...
Now, instead to the Cram\'er-Rao bound  \eqref{eq:gm-one parameter precision bound},
...
We have the QFI matrix $\mathbfcal{F}$ on one hand, which depends on $\rho$ and the two generators $H_0$ and $H_1$, and the covariance matrix from we are only interested on the variance of the gradient parameter, $(\Delta b_1)^2$ \cite{Paris2009}.
Just to record, $H_0$ and $H_1$ are Hermitian operators and commute with each other and thus the QFI matrix elements are computed as $\mathbfcal{F}_{ij}\equiv \qfif{\rho, H_i, H_j},$ following the definition \eqref{eq:gm-FAB}.
...
\be
\label{eq:gm-precision bound for b1 in terms of QFI matrix elements}
(\Delta b_1)^{-2}\leqslant \mathbfcal{F}_{11}-\frac{\mathbfcal{F}_{01}\mathbfcal{F}_{10}}{\mathbfcal{F}_{00}},
\ee
...
In order to be able to saturate the bound, the measurements for optimally estimating the two-parameters must be compatible \cite{Paris2009, Rafal2016}.

To compute the bound Eq.~\eqref{eq:gm-bound-for-insensitive-and-thermal-state}, we will need to simplify the matrix elements in the eigenbasis of the state Eq.~\eqref{eq:gm-eigendecomposition-of-state} of $H_0$ and $H_1$, where the last was already computed in Eq.~\eqref{eq:gm-simplify-h1-matrix-els}.
Hence, we simplify $(H_0)_{\bs{x},\lambda;\bs{y},\nu}$ in a similar way as we did for Eq.~\eqref{eq:gm-simplify-h1-matrix-els}
\be
\begin{split}
...
One can find most of the simplifications of the matrix elements of $\mathbfcal{F}$ in the Appendix~\ref{app:matrix-elements-of-QFI}.
...
\be
  \mathbfcal{F}_{11}=\sum_{n,m}^N \int d^N\bs{x}\prob(\bs{x})x_n x_m \qfif{\rho_{\text{s}}, j_z^{(n)}, j_z^{(m)}}.
\ee
...
\be
  \mathbfcal{F}_{01} = \sum_{n,m}^N \int \prob(\bs{x}) x_m \,\text{d}^N\bs{x}\,
  \qfif{\rho_{\text{s}},j_z^{(n)},j_z^{(m)}}.
\ee
...
\begin{split}
  \varinv{b_1}\leqslant& \sum_{n,m}^N \int d^N\bs{x}\prob(\bs{x})x_nx_m \qfif{\rho_{\text{s}}, j_z^{(n)}, j_z^{(m)}}\\
  &- \frac{\left(\sum_{n=1}^N\int d^N\bs{x}\prob(\bs{x})x_n \qfif{\rho_{\text{s}}, j_z^{(n)},J_z}\right)^2}{\qfif{\rho_{\text{s}}, J_z}}.
\end{split}
\ee

We also whow that show that his bound is invariant under translations as it is the Equation~(\ref{eq:gm-bound-for-insensitive-and-thermal-state}).
With this aim, using the linearity of the last two arguments of $\qfif{\rho, A, B}$, Eq.~(\ref{eq:gm-qfi-linear-in-arguments}), the fact that $H_0$ remains unchanged on the Heisenberg picture and using the shifted $H_1$ operator, Eq.~(\ref{eq:gm-shifted h1 generator}), we have that
\begin{align}
...
\be
\begin{split}
  (\Delta b_1)^{-2}\leqslant\,&\mathbfcal{F}_{11}(d)-\frac{(\mathbfcal{F}_{01}(d))^2}{\mathbfcal{F}_{00}}\\
  =\,& \mathbfcal{F}_{11}-2d\mathbfcal{F}_{01}+d^2\mathbfcal{F}_{00}\\
...
For states of the form Equations~(\ref{eq:gm-separated-internal-and-external},~\ref{eq:gm-thermal-state}), the expression to compute the precision precision bound takes the following form,
...
This bound as well as Equation~(\ref{eq:gm-bound-for-insensitive-and-thermal-state}) is invariant under spatial translations of the system.
...
The first spatial state will be given by $N$ distinguishable ions all placed to a constant distance from the neighbor constituents on a single dimensional configuration, i.e, an ion-chain [Sanah's paper], see Figure~\ref{fig:ionchain-evolution}.
...
\be
  \int d^N\bs{x}\prob(\bs{x}) x_n = na,
\ee
and for the two point average we have the following,
\be
  \int d^N\bs{x}\prob(\bs{x}) x_nx_m = nma^2.
\ee
...
\begin{split}
  (\Delta b_1)^{-2}\leqslant \,&\sum_{n,m}^Nnma^2\qfif{\ket{j}_y^{\otimes N}, j_z^{(n)},j_z^{(m)}}\\
  &-\frac{\left(\sum_{n=1}^N a n \qfif{\ket{j}_y^{\otimes N}, j_z^{(n)},J_z}\right)^2}{\qfif{\ket{j}_y^{\otimes N}, J_z,J_z}}\\
  =\,& a^2 \left\{ \sum_{n=1}^N 2jn^2 - \frac{(\sum_{n=1}^N 2jn)^2}{2jN}\right\}\\
...
Despite that Equation~(\ref{eq:gm-result-for-chain-without-sigma}) is a third order function of the particle number $N$ and that it seems to overcome the ultimate limit HL, on should notice that the length of the chain increases as we introduce more particles into the system.
...
\begin{align}
  \label{eq:gm-mean}
  \mu &= \int d^N\bs{x}\prob(\bs{x}) \frac{\sum_{n=1}^N x_n}{N} \\
  \label{eq:gm-variance}
  \sigma^2 &= \int d^N\bs{x}\prob(\bs{x}) \frac{\sum_{n=1}^N x_n^2}{N} - \mu^2\\
  \label{eq:gm-variance-chain}
  \sigma_{\text{ch}}^2 &= a^2\frac{N^2-1}{12}.
\end{align}
It turns out that this exactly coincides with a factor we have on Equation~(\ref{eq:gm-result-for-chain-without-sigma}).
Substituting this onto the result the proof concludes.

For ion-chains where their constituents are separated by a constant distance and where the spin-state $\rho_{\text{s}}$ is the totally polarised state along a perpendicular direction of both the field and the chain direction, in this case $y$-direction, the precision bound is given by the following formula,
\be
  (\Delta b_1)^{-2}\leqslant 2\sigma_{\text{ch}}^2jN,
\ee
...
\be
  ...
  \prob(\bs{x})=\prod_{n=1}^N \delta(x_n + (-1)^n a),
\ee
...
\be
\begin{split}
  (\Delta b_1)^{-2}|_{\max} &= \sum_{n,m}^N (-1)^{n+m} a^2 \qfif{\ket{\psi}, j_z^{(n)}, j_z^{(m)}}\\
...
% $b_1$, Eq.~\eqref{eq:gm-precision bound for b1 in terms of QFI matrix elements}, is
