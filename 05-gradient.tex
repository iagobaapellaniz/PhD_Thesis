...
%The states of the ensembles are assumed to be permutationally invariant,
...
\be
  \qfif{\rho,A,A}:=\qfif{\rho,A}=2\sum_{k, k'}
  \frac{(\lambda_k-\lambda_{k'})^2}{\lambda_k+\lambda_{k'}}
  |{A}_{k,k'}|^2.
\ee
...
\be
  ...
  (\Delta b_1)^{-2}|_{\max} = \sum_{n,m}^N \int d^N\bs{x}P_N(\bs{x})x_n x_m \qfif{\rho_{\rm s}, j_z^{(n)}, j_z^{(m)}},
\ee
...
\be
  \mathbfcal{F}_{11}=\sum_{n,m}^N \int d^N\bs{x}P_N(\bs{x})x_n x_m \qfif{\rho_{\rm s}, j_z^{(n)}, j_z^{(m)}}.
\ee
...
\be
  \mathbfcal{F}_{00} = \qfif{\rho_{\rm s}, J_z}.
\ee
...
\be
  \mathbfcal{F}_{01} = \sum_{n=1}^N \int d^N\bs{x}P_N(\bs{x})x_n\qfif{\rho_{\rm s}, j_z^{(n)},J_z}.
\ee
...
\begin{split}
  (\Delta b_1)^{-2}\leqslant& \sum_{n,m}^N \int d^N\bs{x}P_n(\bs{x})x_nx_m \qfif{\rho_{\rm s}, j_z^{(n)}, j_z^{(m)}}\\
  &- \frac{\left(\sum_{n=1}^N\int d^N\bs{x}P_n(\bs{x})x_n \qfif{\rho_{\rm s}, j_z^{(n)},J_z}\right)^2}{\qfif{\rho_{\rm s}, J_z}}.
\end{split}
\ee
...
For ion-chains where their constituents are separated by a constant distance and where the spin-state $\rho_{\rm s}$ is the totally polarised state along a perpendicular direction of both the field and the chain direction, in this case $y$-direction, the precision bound is given by the following formula,
\be
...
% &&=\sum_{n,m}^{N}\qfif{\rho_{\rm s},j_z^{(n)},j_z^{(m)}}
...
% $\qfif{\rho_{\rm s},J_z}$ is different from zero.
%
...
% =\sigma^2
% \sum_{n=1}^N
% \qfif{\rho_{\rm s},j_z^{(n)}}
% + \eta
% \sum_{n \neq m}^N
% \qfif{\rho_{\rm s},j_z^{(n)},j_z^{(m)}}.
% %=N (\sigma^2-\eta)\qfi[\rho_{\rm
% %s},j_z^{(1)},j_z^{(1)}]+\eta \qfif{\rho_{\rm s},J_z,J_z}.
% \ee
% After using the facts that the following identity holds for any state,
% $\qfif{\rho_{\rm s}, J_z}=N\qfi[\rho_{\rm
% s},j_z^{(1)}]+N(N-1)\qfif{\rho_{\rm s},j_z^{(1)},j_z^{(2)}}$,
...
(\Delta b_1)^{-2}\leqslant (\sigma^2-\eta) \sum_{n=1}^N \qfi[\rho_{\rm
s},j_z^{(n)}]+\eta \qfif{\rho_{\rm s},J_z}.
\ee
...
% $\qfif{\rho_{\rm s},J_z,J_z}$, is maximized and when the covariance is
% maximized too.
...
Therefore, the separable inner state that maximizes the precision must be a pure product state that maximises all possible $\qfif{\rho_{\rm s}, j_z^{(n)}, j_z^{(m)}}$.%, see Fig.~\ref{fig:convexity in separable states of qfi}.
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=244pt]{img/GM_states_representation.pdf}
\caption[Different set of states and its first and second moments representation.]{Angular momentum components and their variances for various quantum states.
