...
[TODO: Add additive under tensor product property].
\be
...
This observations make the computations of the precision bounds in the next sections easier, since the now we can place the system arbitrarily wherever we choose.
...
\ee
where the summation is over all the possible permutations of the variables $x_n$ denoted by $\Pi_k$.
...
\be
  \int x_n \prob(\bs{x}) \, d^N\bs{x} = \int \frac{\sum_{n=1}^N x_n}{N} \prob(\bs{x}) \, d^N\bs{x} = \mu = 0,
...
\be
  \int x_n x_m \prob(\bs{x}) \, d^N\bs{x} = \lcor
  \begin{aligned}
...
\ee
Note that in the first sum in Eq.~\eqref{eq:gm-bound-for-sensitive-and-thermal-state} there are $N(N-1)$ terms proportional to $\eta/(N-1)$ and $N$ terms proportional to $\sigma^2$.
...
\ee
which will be a useful identity later on.
...
\be
\begin{split}
  (\Delta b_1)^{-2}|_{\max} &= \sum_{n,m}^N \int d^N\bs{x}\prob(\bs{x})x_n x_m \qfif{\rho^{(\text{s})}, j_z^{(n)}, j_z^{(m)}}\\
...
\be
\begin{split}
  \varinv{b_1}\leqslant& \sum_{n,m}^N \int d^N\bs{x}\prob(\bs{x})x_nx_m \qfif{\rho^{(\text{s})}, j_z^{(n)}, j_z^{(m)}}\\
  =& \sum_{n=1}^N \sigma^2 \qfif{\rho,j_z^{(n)}} + \sum_{n\neq m}^N \eta \qfif{\rho,j_z^{(n)},j_z^{(m)}}\\
  =&(\sigma^2- \eta)\sum_{n=1}^N  \qfif{\rho,j_z^{(n)}} + \eta\sum_{n, m}^N  \qfif{\rho,j_z^{(n)},j_z^{(m)}},
\end{split}
\ee
where in the second line we compute the diagonal and the off-diagonal terms of the sum separately and in the last line we add $\eta\sum_{n=1}^N \qfif{\varrho,j_z^{(n)}}$ to the last term and subtract it from the first term to make the expression more similar to Eq.~\eqref{eq:gm-bound-insensitive-sinens}.
...
\ee
The second term on the right-hand side of Eq.~\eqref{eq:gm-bound-sensitive-sinens} is new in the sense that it did not appear on the bound for states insensitive to homogeneous fields.
...
\begin{table}
  ...
  \begin{tabular}{|l|c|}
    ...
    \hline
    $\ket{\ghz}$ & $|_{\max} =  (\sigma^2 - \eta) N  + \eta N^2$ \\
    \hline
  \end{tabular}
  \end{center}
...
\be
\rho^{(\text{s})}=\sum_{D=1}^{D_0}\lambda_D\ket{0,0,D}\bra{0,0,D},
...
\be
  ...
  =\tr(\rho^{(\text{s})}(j_l^{(n)})^2),
\ee
for any pair $k,l\in x,y,z$ due to the rotational invariance of the singlet, i.e, all the singlets remain invariant under a $SU(2)$ transformation of the kind $U=e^{i\phi J_{\vec n}}$, where $\vec{n}$ is an unitary vector belonging to the positional space.
...
[TD: ADD OPTIMAL MEASUREMENT APPENDIX]
...
For the gradient measurement as for the measurement of the homogeneous field, the polarization must be perpendicular to the field we want to measure in order to take advantage of the interaction between the particles and the field.
Here we chose as before the totally polarized state along the $y$-axis which is written as $\ket{j}_y^{\otimes N}$.
note that this state is a state sensitive to the homogeneous field, hence, we must use the Equation~(\ref{eq:gm-bound-sensitive-sinens}) to compute the bound.
...
of the totally polarized state as follows,
\be
...
Note that we can place the system at the point on which the magnetic field is null without changing the result.
...
On the other hand, for pure product states we have first that $\qfif{\rho^{(\text{s})}, j_z^{(n)}, j_z^{(m)}}$ is four times the correlation between the single particle spin operators, which using the well known properties of the pure product states is zero when $n \neq m$.
$\eta$ does not play any role in the precision then.
Finally, we have to maximize only the variance of single particle operators $4(\Delta j_z^{(n)})^2$.
...
This states gives $(\Delta j_z^{(n)})^2=j^2$ which corresponds to a quarter of the $\qfif{\rho^{(\text{s})}, j_z^{(n)}, j_z^{(n)}}$ appearing into the Eq.~\eqref{eq:gm-f11-thermal},
where inner state for all particles is just the product state $\rho^{(\text{s})} =(\ketbra{\Psi}{\Psi})^{\otimes N}$.
...
The best achievable precision for separable states is written as
\be
  \varinv{b_1} \leqslant 4N j^2 \sigma^2,
...
The unpolarized Dicke state $\ket{\dicke{N}}_l$ with a maximal $\expect{J_x^2+J_y^2+J_z^2}=\mathcal{J}_{N/2}$, defined in Eq.~\eqref{} and $\langle J_l\rangle=0$ for any $l\in x,y,z$ is particularly interesting due to its entanglement properties and its metrological usefulness.
...
Moreover it is very useful for homogeneous magnetometry as it has been shown in Ref.~\cite{GMLS11}.
...
This is due to the metrological enhancement the state has for the sensing of the homogeneous field.
...
We invoke the definition of the GHZ states Eq.~\eqref{eq:ghz} as
\be
...
\varinv{b_1} = (\sigma^2 - \eta) N  + \eta N^2.
\ee
...
[TD: ADD COMPATIBILITY OF MEASUREMENTS]
