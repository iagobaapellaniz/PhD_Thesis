...
More concretelly, we can measure the expectation values of any direction
\be
...
Following arguments found on Refs.~\citep{Giovannetti2004, Paris2009}, one can reach to an universal bound valid for any kind of measurements.
...
The first is based on the eigen-decomposition of the state whereas the second is the convex-roof of $4\varian{J_z}$ \citep{Paris2009, Toth2013, Yu2013}.
...
It is also instructive to define a QFI averaged on all possible directions.
Simple calculations show that
\be
  \label{eq:bg-average-qfi}
  \underset{\bs{n}}{\text{avg}}\,\qfif{\rho, J_{\bs{n}}} \equiv \int_{|\bs{n}|=1}\qfif{\rho,J_{\bs{n}}}\,\text{d}\bs{n} = \frac{1}{3}\sum_{l=x,y,z} \qfif{\rho,J_l}.
\ee
...
Reference~\citep{Hyllus2012} presented states that were detected as bound entangled based on the criterion tor the average QFI Eq.~\eqref{eq:bg-average-qfi}.
Reference~\citep{Czekaj2014} presented states that violate the criterion based on a bound for the quantum Fisher information similar to Eq.~\eqref{eq:bg-entanglement-criterion-qfi}.
