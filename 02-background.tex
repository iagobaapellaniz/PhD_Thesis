...
For a more detailed material, check statistics book as well as mathematics book for scientist and engineers \citep{Riley2006, Barlow1989}}.
...
The following References~\citep{Riley2006, Barlow1989} are some of those books.

When someone has some data sample at hand, it might come from diverse sources and might be represented using multiple forms such as numbers or words (for instance, a table of names of people).
...
Focusing into what it is more interesting for the comprehension of this thesis, an estimator of any kind has a theoretical lower bound for its variance.
...
A \emph{separable} state in this Hilbert space can be written by
\be
...
For a single party with discrete $d$ levels, and therefore the spin-number $s=(d-1)/2$, the eigenvalue equation for the angular momentum projection operators are
\be
  j_l^{(n)}\ket{m}_l^{(n)} = m \ket{m}_l^{(n)}
\ee
for $m=-s,\dots,+s$, where $l=x,y,z$.
It is usual to omit the subscript of $\ket{m}_l^{(n)}$ when $l=z$ because it is the preferred direction for many authors and the superscript ($n$) can be also omitted when the single-particle states are given in an ordered form to build a multiparticle state, i.e., $\ket{\psi} = \ket{m_1}\ket{m_2}\dots\ket{m_n}$ where we also omitted on writing the tensor product operator $\otimes$ in between single-particle states.
In many cases it is also usual to merge all into a single, but at the same time with many quantum numbers, state $\ket{\psi}=\ket{m_1 m_2 \dots m_N}$ for simplicity\footnote{
This notation is normally used in many-body quantum mechanics, see diffetent textbooks Refs.~\citep{Cohen-Tannoudji1977, Sakurai2014}.}.

The square of the total angular momentum, $\bs{j}^2=j_x^2+j_y^2+j_z^2$, for a single party $(n)$ acts on any state simply as
\be
  (\bs{j}^2)^{(n)}\rho=s_n(s_n+1)\rho,
\ee
where state must be defined in a Hilbert space containing $\mathcal{H}^{(n)}$, the Hilbert space of $n^{\text{th}}$ particle and where $s_n$ is the spin number of such a particle.
...
\ee
where $\mtxid$ stand for the identity operator and the last formula is the most used since it is easy to notice that the single-party operator must extended in order to sum the correctly.
On the other hand, notice that the square of the different projections of total angular momentum are not equal to the sum of the square angular momentum projections of each of the parties.
Thus it is obtained by
\be
  J_l^2 = \sum_{i,j}^N j_l^{(i)} j_l^{(j)} = \sum_{i=1}^N (j_l^2)^{(i)}+\sum_{i\neq j}^N j_l^{(i)} j_l^{(j)}.
\ee
Therefore, neither the total square angular momentum is the sum of all single-party square angular momentum but
\be
  \bs{J}^2 = \sum_{i}^N \bs{j}^{(i)} + \sum_{l=x,y,z}\sum_{i\neq j}^N j_l^{(i)} j_l^{(j)},
\ee
where we separated the sum into two pieces, the first corresponds to the sum of all single party angular momentum projections squared and the second corresponds to the product of angular momentum projection operators of two distinct subsystems summed for all $l=x,y,z$.
Many more combinations of these single-party operators may arise on different contexts.
In the Appendix~\ref{app:mima-angular-subspaces}, we discuss in more detail the different structures that arise from adding the angular momentum operators, e.g., the symmetric subspace, the fixed total angular momentum subspace or the singlet subspace.
...
These transformations are govern by master-equations of the type of the Lindblad equation \citep{Lindblad1976, Nielsen2000, Breuer2002}.
...
One can check different textbooks of quantum mechanics, just to mention some see Ref.~\citep{Cohen-Tannoudji1977, Sakurai2014}.

\subsection{Quantum Metrology}
\label{sec:bg-quantum-magnetometry}

We summarize important recent advances in quantum metrology.
Simple metrological setups allow for encoding some wanted parameter into the state of the system, thus from the readout of the final state one could infer it.
...
Nevertheless, with the initial pioneering works of W. K. Wotters, and S. L. Braunstein and C. M. Caves, in 1981 and 1992-1994 respectively \citep{Wotters1981, Braunstein1992, Braunstein1994}, until the works of V. Giovannetti et al, and M. G. Paris, roughly two decades later \citep{Giovannetti2004, Paris2009}, the field of quantum metrology was stabilized with solid foundations.
% TD: important citations needed here!!
Later, advanced works in quantum metrology appeared \citep{XXX} together with experimental realizations \citep{Behbood2013, Koschorreck2011, Lucke2011} which raised the interest in this topic.
In this section, we will highlight the most important aspects of this field and with this we will conclude this chapter for presenting the background theory on which sits the work developed by the authors.
...
Finally, the outgoing state is characterized by some measured quantity $\expect{M}$, which allows to infer in the value of the parameter $\theta$.
...
The equation above takes the name \emph{shot-noise} or standard scaling (SNS), the term originating from the shot-noise in electronic circuits, which is due to the discrete nature of the electric charge.
...
This is justified in the sense that most of the recent experiments on this context have been carried out with ion-traps, Bose-Einstein condensates (BEC) or at most cold atomic ensembles, which have indeed a negligible orbital angular momentum.
...
\ee
where $\expect{M}$ is the expectation value of the operator $M$, and we used the assumption that $\expect{M}$ is a random variable and that it has a PDF that can relate $\prob(\expect{M}|\theta)$ with a PDF as a function of the estimator $\prob(\hat{\theta}|\theta)$, see the Section~\ref{} and for a recent review discussing this approach in detail, see Ref.~\citep{Kolodynski2010}.
...
Such states are called \emph{spin-squeezed} states \citep{Kitagawa1993, Wineland1994, Sorensen2001, Ma2011} and they can in principal bite the SNS.
...
In this section, we review the theoretical background of the Fisher information, the Cram\'er-Rao bound and we introduce the quantum Fisher information (QFI).
...
Following arguments found on Refs.~\citep{Giovanneti2001,Paris2009}, one can reach to an universal bound valid for any kind of measurements.
This bound is called the quantum Cram\'er-Rao bound and in one site is the achievable precision and in the other site is the quantum Fisher information,
\be
...
Fortunately, the Eq.~\eqref{eq:bg-quantum-cr-bound} gives us that upper bound that is valid for any choice on the measurement \citep{Helstrom1976, Holevo1994}.

As a direct consequence, based on Eq.~\eqref{eq:bg-error-propagation-formula} for any given $M$ we have that
\be
  \qfi[\rho,J_z] \geq \frac{|\partial_{\theta}\expect{M}|^2}{\varian{M}}
\ee
...
The first is based on the eigen-decomposition of the state whereas the second is the convex-roof of $4\varian{J_z}$ \citep{Paris2009, XXX, Geza}.
Both formulas yield to the same expression for pure states in an easy way
\be
  \qfi[\ket{\psi}, J_z] = 4\varian{J_z},
\ee
...
\begin{enumerate}
  ...
  \be
    \qfi[p\rho_1+(1-p)\rho_2, J_z]\leq p\qfi[\rho_1, J_z]+(1-p)\qfi[\rho_2, J_z]
  \ee
  ...
  \item For all states, the QFI is smaller or equal to four times the variance in all cases,\be \qfi[\rho,J_z] \leq 4\varian{J_z}_\rho.\ee.
\end{enumerate}
...
\be
  \frac{1}{4}\qfi[\rho,J_z]\leq \sum_{k}p_k \varian{J_z}_{\Psi_k}\leq \varian{J_z}_{\Psi_k},
\ee
...
\begin{enumerate}
  ...
  \be
    \qfi[U\rho U^\dagger, A] = \qfi[\rho, U^\dagger A U].
  \ee
  ...
  \be
    \qfi[\rho^{(1)}\otimes \rho^{(2)}, A^{(1)}\otimes \mtxid^{(2)}+\mtxid^{(1)}\otimes A^{(2)}] = \qfi[\rho^{(1)},A^{(1)}]+ \qfi[\rho^{(2)},A^{(2)}].
  \ee
  ...
  \be
    \qfi[{\textstyle\bigoplus_{k}p_k \rho_k,\bigoplus_{k} A_k}] = \sum_{k}p_k\qfi[\rho_k,A_k],
  \ee
  where $\sum_k p_k = 1$.
  The above equation is relevant, for instance, for experiments where the particle number variance is not zero, and the $\rho_k$ corresponds to density matrices with a fixed particle number \citep{Hyllus2010, Hyllu2012}.
\end{enumerate}
...
\be
  \qfi[\rho,J_z] = 4\varian{J_z} = 4 \sum_{n=1}^N \varian{j_z^(n)} \leq N.
\ee
...
\be
  ...
  \qfi[\rho, J_z] = 4 \varian{J_z}\leq N^2,
\ee
...
Equation~\eqref{eq:bg-shot-noise-limit} has been used already to detect entanglement based on the metrological performance of the quantum states in Refs.~\citep{Krischek2011, Lucke2011}.
