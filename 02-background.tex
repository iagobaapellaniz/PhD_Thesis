\section{Background in quantum metrology}
...
Enhanced by initial developments of quantum technologies, quantum information attracted much attention from the scientific community.
Moreover, those emerging fields rapidly became into a very interesting interdisciplinary playground of science with many scientists as well as resources involved.

Focusing on quantum metrology, the role of entanglement, an exclusive feature of quantum mechanics which cannot be described using classical probabilistic theories, is essential.
Entanglement is also at the center of quantum metrology, and it connects quantum metrology with concepts like local realism or quantum teleportation.
Nevertheless, throughout the thesis we will focus mainly on the achievable precision for different systems and schemes.

On the other hand, we will also present some important tools and concepts of statistic, without which many descriptions of the actual and past physical findings would lack of the rigorous interpretation needed.
...
Once we have a data sample, we normally face the task of analyzing the data to extract the relevant properties from it.
...
When some indirect property of the system is defined as a function of the measured random variables, the result is also a new random variable with another assigned PDF.
...
Some definitions, useful to mention in this thesis, arise for these kind of data samples: namely the average, variance, and the statistical moments and central moments.
The arithmetic average (there are other types of averages one can find in the literature) is computed as
\be
...
An estimator is a function of the data sample.
...
Focusing now onto what it is more interesting for this thesis, an estimator of any kind has a theoretical lower bound for its variance.
...
\be
  ...
  \text{V}[\hat{a}] = \int (\hat{a} - a)^2\prob(\bs{x}|\bs{a})\,\text{d}^N\bs{x} \geqslant \frac{1}{\int \lpar\partial_a\big[\ln \prob (\bs{x}|\bs{a}) \big]\rpar^2 \prob (\bs{x}|\bs{a})\,\text{d}^N\bs{x}},
\ee
...
\begin{enumerate}
  ...
  \item Using the properties (iii) and (iv), it follows that the quantum states form a convex set, where the extreme must be composed of pure states.
...
We now present a set of operators that will appear many times in all chapters, namely the angular momentum operators.
...
The collective angular momentum projection operators $J_l$ are defined as the sum of their respective single-party spin operators $j_l^{(n)}$ such that they are extended to the remaining of the Hilbert spaces by tensor products of the identity operators defined in the rest of subspaces,
\be
...
\ee
where $\mtxid$ stand for the identity operator and where in the last equality the identity operators are omitted for simplicity.
...
In the Appendix~\ref{app:angular-subspaces}, we discuss in more detail the different structures that arise from adding the angular momentum operators for different parties, e.g., the symmetric subspace or the singlet subspace.
...
Simple metrological setups allow for encoding some wanted parameter into the quantum state.
...
The magnetic moment of the system is proportional to the total angular momentum, $\bs{\mu}=-\mu_\text{B} g_{\text{s}}\hbar \bs{J}$, where $\mu_{\text{B}}$ and $g_{\text{s}}$ are the Bohr magneton and the anomalous gyromagnetic factor respectively, and where we multiply it by the Plank reduced constant $\hbar$.
Note that throughout all the thesis we use the angular momentum components derived from the single-particle angular momentum components Eq.~\eqref{eq:bg-single-particle-am-components}, which were defined with $\hbar{=}1$ for simplicity.
...
Such collective quantities are in this case the total angular momentum components defined in Eq.~\eqref{eq:bg-total-angular-momentum-progection-operators} and their combinations.
...
We focus on multi-partite systems of spin-$\frac{1}{2}$ particles, widely known as qubits.
...
\ee
where we introduce a notation for trigonometric functions such that $\coss{x} \rightarrow \cos(x)$, $ \sins{x}\rightarrow\sin(x)$ and $\tans{x}\rightarrow\tan(x)$.
...
\be
  \varian{J_x}\varian{J_z} \geqslant \frac{1}{4}|\expect{J_y}|^2.
\ee
...
Next, we can ask, what is the best possible phase estimation precision is for the metrological task considered in this section.
...
\ee
where $J_{\max}$ is the maximum value an angular momentum component can take, i.e., $N/2$.
...
Some interesting properties of the quantum Fisher information emerge from these expressions and we summarize them in the following list:
\begin{enumerate}
  ...
  \ee
  \item Recently, it has been shown due to Eq.~\eqref{eq:bg-qfi-definition-convex-roof}, that the quantum Fisher information is the largest convex function that fulfills Eq.~\eqref{eq:bg-qfi-for-pure-states} \cite{Toth2013, Yu2013}.
  ...
  \item For all states, the QFI is smaller or equal to four times the variance \be
    \qfif{\rho,J_z} \leqslant 4\varian{J_z}_\rho.
  \ee
\end{enumerate}
It can be proven that the variance is the smallest concave-roof of the variance itself \cite{Toth2013}.
...
\be
  \frac{1}{4}\qfif{\rho,J_z}\leqslant \sum_{k}p_k \varian{J_z}_{\ket{\psi_k}}\leqslant \varian{J_z}_{\rho},
\ee
...
It has been shown that there are even highly entangled pure states which are not useful.
Hence, the presence of entanglement seems to be rather a necessary but not sufficient condition.

The quantum Fisher information can be used to define a entanglement parameter that characterizes the metrological usefulness by
\be
...
On the other hand, for $k$-producible states the QFI is bounded from above, similarly as we did in Eqs.~\eqref{eq:bg-shot-noise-limit} and \eqref{eq:bg-heisenberg-limit}, as \cite{Hyllus2012, Toth2012}
\be
  \begin{split}
